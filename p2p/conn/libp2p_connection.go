...
// recvRoutineChannel reads PacketMsg and reconstructs the message using the channels' "recving" buffer.
// After a whole message has been assembled, it calls onReceive() serially.
// Blocks depending on how the connection is throttled.
// Otherwise, it never blocks.
func (c *Libp2pMConnection) recvRoutineChannel(ch *Libp2pChannel) {
	...
	protoReader := protoio.NewDelimitedReader(r, c._maxPacketMsgSize)
	...
	for {
		...
		*/
		...
		case *tmp2p.Packet_PacketMsg:
			msgBytes, err := ch.recvPacketMsg(*pkt.PacketMsg)
			if err != nil {
				if c.IsRunning() {
					c.Logger.Error("Connection failed @ recvRoutine", "conn", c, "err", err)
					c.stopForError(err)
...
// call MConnection.maxPacketMsgSize() directly
...
// Not goroutine-safe
func (ch *Libp2pChannel) nextPacketMsg() tmp2p.PacketMsg {
...
func (ch *Libp2pChannel) writePacketMsgTo(w io.Writer) (n int, err error) {
...
// Not goroutine-safe
func (ch *Libp2pChannel) recvPacketMsg(packet tmp2p.PacketMsg) ([]byte, error) {
