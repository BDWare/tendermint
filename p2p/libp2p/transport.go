...
import (
	...
	"github.com/tendermint/tendermint/p2p/libp2p/util"
)
...
func handshake(
	...
	go func(errc chan<- error, s network.Stream) {
		...
		_, err := protoio.NewDelimitedWriter(s).WriteMsg(ourNodeInfo.ToProto())
		errc <- err
	}(errc, s)
	...
	go func(errc chan<- error, s network.Stream) {
		...
		//)
		protoReader := protoio.NewDelimitedReader(s, p2p.MaxNodeInfoSize())
